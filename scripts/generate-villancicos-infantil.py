...
def save_cards_to_markdown(cards, songs, categoria):
    ...
    # Carpeta de destino
    folder_name = "villancicos-infantil"
    base_path = Path(__file__).parent.parent / 'cartones' / folder_name
    base_path.mkdir(parents=True, exist_ok=True)
    
    # 1. Archivo de listado de canciones
    listado_path = base_path / 'listado-canciones-villancicos-infantil.md'
    listado_content = f'# Listado de Canciones - {categoria}\n\n'
    listado_content += f'**Total:** {len(songs)} villancicos navideños\n\n'
    ...
    cartones_md_path = base_path / 'cartones-villancicos-infantil.md'
    ...
    cartones_content += f'- 🎄 {len(songs)} villancicos en total\n\n'
    cartones_content += '---\n\n'
    ...
    cartones_txt_path = base_path / 'cartones-villancicos-infantil.txt'
...
def main():
    """Función principal"""
    print('\n🎄 Generador de Cartones de Bingo Musical - Villancicos Infantil\n')
    print(f'📊 Configuración:')
    print(f'   - Canciones totales: {len(VILLANCICOS)}')
    ...
    combinaciones_posibles = comb(len(VILLANCICOS), CANCIONES_POR_CARTON)
    ...
    cards = generate_unique_cards(VILLANCICOS, CANCIONES_POR_CARTON, NUM_CARTONES)
    print(f'✅ {len(cards)} cartones generados correctamente\n')
    
    # Guardar archivos
    print('💾 Guardando archivos...')
    result = save_cards_to_markdown(cards, VILLANCICOS, CATEGORIA)
    ...
    print(f'\n📊 Resumen:')
    print(f'   ✅ {len(VILLANCICOS)} villancicos')
